...
The columns *Year*, *Month*, *Day*, and *Day_Frac* are together forming a 
*time stamp*, and our first duty is to introduce the **R** *Date* format:
...
The next step is to add the *DayFrac* to *date* (internally, a *Date* is just 
a numeric value). Before that we must change *NA* to 0 (zero). Finally 
*DayFrac* is removed from *events*
...
So there are no duplicated rows in *events*. Next we create new variables from 
the *Type* and  *Value* columns with *tidyr::spread*.
...
Then we need to "fill down" missing values, by *Id_I*.

```{r filldownidi}
varns <- unique(events$Type)
for (id in unique(episodes$Id_I)){
    who <- episodes$Id_I == id
    epi <- tidyr::fill_(episodes[who, ], fill_cols = varns)
    episodes[who, ] <- epi
}
knitr::kable(head(episodes))
```
